...
const Products = () => {
  ...
  const { data: productsData, isLoading, error } = useQuery({
    queryKey: ['products', currentPage, itemsPerPage, searchTerm, selectedCategory, selectedProductType, selectedSize, stockFilter, statusFilter, sortBy, sortOrder],
  ...
  const createVariantsMutation = useMutation({
    mutationFn: (data) => productsAPI.createVariants(data),
    onSuccess: (response) => {
      queryClient.invalidateQueries(['products']);
      setShowVariantForm(false);
      const summary = response.data.summary;
      toast.success(`Created ${summary.total_variants} variants for "${summary.base_name}" (${summary.sizes.join(', ')})!`);
    },
    onError: (error) => {
      toast.error(error.response?.data?.error || 'Failed to create product variants');
    }
  });
  ...
  const handleBulkDelete = () => {
    ...
    if (confirm(`Are you sure you want to delete ${selectedItems.size} products?`)) {
    ...
    <div className="space-y-4 sm:space-y-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-sm sm:text-base text-gray-600 mt-1 sm:mt-2">Manage your product catalog and inventory</p>
          {(() => {
            const user = JSON.parse(localStorage.getItem('user') || '{}');
            const isAdmin = user.role === 'admin';
            return (
              <div className="mt-2">
                <span className={`inline-flex items-center px-2 py-1 text-xs font-medium rounded-full ${isAdmin
                  ...
                  {isAdmin ? '🌐 Global Inventory View' : '🏪 Shop Inventory View'}
                </span>
              </div>
            );
          })()}
        ...
        <div className="flex items-center space-x-3">
          {(() => {
            ...
            const isAdmin = user?.role === 'admin';
            const isManager = user?.role === 'manager';
            const canGenerateBarcodes = isAdmin || isManager;
            return (
              <>
                {/* Manual Assign Clothes to Gisimete Button */}
                {(isAdmin || isManager) && (
                  <button
                    onClick={async () => {
                      if (confirm('Are you sure you want to assign all "Clothes" products to the Gisimete shop? This will create missing assignments for existing products.')) {
                        try {
                          const promise = productsAPI.assignClothesToGisimete();
                          toast.promise(promise, {
                            loading: 'Assigning products...',
                            success: (res) => `Assigned ${res.data.stats.assigned} new products!`,
                ...
                <>
                  <span className="text-sm font-medium text-gray-900">
                    {stockStatus.isShopSpecific ? (
                      ...
                      <span className="text-gray-900 font-semibold">{stockStatus.quantity} Unit</span>
                    )}
                  </span>
                  {stockStatus.totalShopQuantity !== undefined && (
                    <div className="text-xs text-gray-500 mt-1">
                      <div>Total in shops: {stockStatus.totalShopQuantity} Unit</div>
                      <div>Available in {stockStatus.shopsWithProduct} shop(s)</div>
                      {stockStatus.availableForAssignment !== undefined && (
                        <div className="text-green-600 font-medium">Available for Assignment: {stockStatus.availableForAssignment} Unit</div>
                      )}
                    </div>
                  )}
                </>
...
const QuickViewModal = ({ product, onClose, onEdit, formatPrice, getCategoryNames }) => {
  ...
  const stockStatus = displayQuantity === 0 ? 'Out of Stock' :
    displayQuantity <= displayMinStock ? 'Low Stock' : 'In Stock';
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${stockStatus === 'Out of Stock' ? 'bg-red-100 text-red-800' :
...
const ProductModal = ({ product, onClose, onSubmit, isLoading, categories, brands }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    // SKU is no longer required since uniqueness constraint is removed
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg shadow-xl max-w-4xl w-full mx-2 sm:mx-4 max-h-[98vh] sm:max-h-[90vh] overflow-y-auto p-2 sm:p-4">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
              ...
              <div>
                ...
                {formData.barcode && (
                  <div className="mt-2 p-3 bg-gray-50 rounded-lg">
                    <label className="block text-sm font-medium text-gray-600 mb-2">Barcode Preview</label>
                    <BarcodeDisplay
                      value={formData.barcode}
                      width={1.5}
                      height={32}
          ...
          {/* Pricing & Inventory */}
          <div>
            <h4 className="text-md font-semibold text-gray-900 mb-4">Pricing & Inventory</h4>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <CurrencyInput
                  ...
                  placeholder="0.00"
                  error={formData.price && formData.cost_price && parseFloat(formData.cost_price) >= parseFloat(formData.price) ? "Cost price must be less than selling price" : ""}
                />
                {formData.price && formData.cost_price && parseFloat(formData.cost_price) >= parseFloat(formData.price) && (
                  <p className="mt-1 text-sm text-red-600">
                    ⚠️ Cost price must be less than selling price ({formData.price} RWF)
                  </p>
                )}
              </div>
              ...
              <div>
                <CurrencyInput
                  ...
                  placeholder="0.00"
                  required
                  error={formData.cost_price && formData.price && parseFloat(formData.price) <= parseFloat(formData.cost_price) ? "Selling price must be greater than cost price" : ""}
                />
                <p className="text-xs text-gray-500 mt-1">This will be used as default for all variants</p>
                {formData.cost_price && formData.price && parseFloat(formData.price) <= parseFloat(formData.cost_price) && (
                  <p className="mt-1 text-sm text-red-600">
                    ⚠️ Selling price must be greater than cost price ({formData.cost_price} RWF)
                  </p>
                )}
              </div>
          ...
          {/* Product Attributes */}
          <div>
            <h4 className="text-md font-semibold text-gray-900 mb-4">Product Attributes</h4>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <input
                  ...
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                  placeholder="e.g., S, M, L, XL, 30ml, 50ml"
                />
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                  placeholder="e.g., Limited Edition, Premium"
                />
              </div>
          ...
          {/* Image Upload */}
          <div>
            <h4 className="text-md font-semibold text-gray-900 mb-4">Product Image</h4>
...
const ProductVariantModal = ({ onClose, onSubmit, isLoading, categories, brands }) => {
  const [formData, setFormData] = useState({
    name: '',
    description: '',
    brand_id: '',
    ...
    image_url: '',
    variants: [
      { size: 'S', price: '', stock_quantity: '', min_stock_level: '' },
      { size: 'M', price: '', stock_quantity: '', min_stock_level: '' },
      { size: 'L', price: '', stock_quantity: '', min_stock_level: '' },
      { size: 'XL', price: '', stock_quantity: '', min_stock_level: '' }
    ]
  });
  ...
  const addVariant = () => {
    setFormData({
      ...formData,
      variants: [...formData.variants, { size: '', price: '', stock_quantity: '', min_stock_level: '' }]
    });
  };
  ...
  const handleCustomSizesChange = (e) => {
    ...
    if (sizes.trim()) {
      ...
      const newVariants = sizeArray.map(size => ({
        size,
  ...
  const handleSubmit = (e) => {
    ...
    const submitData = {
      ...
      variants: formData.variants.map(variant => ({
        ...variant,
        price: parseFloat(variant.price) || parseFloat(formData.base_price) || 0,
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg shadow-xl max-w-4xl w-full mx-2 sm:mx-4 max-h-[98vh] sm:max-h-[90vh] overflow-y-auto p-2 sm:p-4">
        <div className="flex items-center justify-between p-6 border-b border-gray-200">
          <h3 className="text-lg font-semibold text-gray-900">Create Product Variants</h3>
          <button onClick={onClose} className="text-gray-400 hover:text-gray-600">
            <X className="h-6 w-6" />
          </button>
        </div>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          ...
          <div>
            <h4 className="text-md font-semibold text-gray-900 mb-4">Base Product Information</h4>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
          ...
          {/* Pricing */}
          <div>
            <h4 className="text-md font-semibold text-gray-900 mb-4">Base Pricing</h4>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">Cost Price (RWF)</label>
                <input
                  type="number"
                  step="0.01"
                  min="0"
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">Base Price (RWF)</label>
                <input
                  type="number"
                  step="0.01"
                  min="0"
          ...
          {/* Size Configuration */}
          <div>
            <h4 className="text-md font-semibold text-gray-900 mb-4">Size Configuration</h4>
            <div className="mb-4">
              <label className="flex items-center">
                ...
                />
                <span className="ml-2 text-sm text-gray-700">Use custom sizes</span>
              </label>
            </div>
              ...
              {formData.variants.map((variant, index) => (
                <div key={index} className="border border-gray-200 rounded-lg p-4">
                  <div className="flex items-center justify-between mb-3">
                    ...
                    {!useCustomSizes && formData.variants.length > 1 && (
                      <button
                        type="button"
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) => handleVariantChange(index, 'min_stock_level', e.target.value)}
                        className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-green-500"
                        placeholder="0"
                      />
                    </div>
          ...
          <div className="flex items-center justify-end space-x-3 pt-4">
            ...
            >
              ...
              {isLoading ? 'Creating...' : `Create ${formData.variants.length} Variants`}
            </button>
          </div>
        </form>
      </div>
