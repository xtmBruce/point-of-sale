...
const OrderForm = ({ onOrderCreated, onClose, isFullPage = false, modernPOS = false }) => {
  ...
  const sortedProducts = [...filteredProducts].sort((a, b) => {
    ...
    switch (sortField) {
      ...
      case 'name':
      default:
        // Primary sort by name with numeric awareness (so "30ml" comes before "100ml")
        const nameComparison = (a.name ?? '').localeCompare(b.name ?? '', undefined, { numeric: true, sensitivity: 'base' });
        if (nameComparison !== 0) return nameComparison * dir;

        // Secondary sort by size (extract numeric value if possible)
  ...
  const isGiftoPayment = paymentMethod === 'gifto' || (isSplitPayment && splitPayments.every(p => p.method === 'gifto'));
  ...
  // Gift card verification function
  const verifyGiftCard = async (cardNumber) => {
    console.log('Verifying gift card:', cardNumber, 'Length:', cardNumber?.length);
    
    if (!cardNumber || cardNumber.length !== 16) {
      toast.error('Please enter a valid 16-digit gift card number');
      return;
    }

    // Additional validation - ensure it's all digits
    if (!/^\d{16}$/.test(cardNumber)) {
      toast.error('Gift card number must contain only digits');
      return;
    }

    setCheckingGiftCard(true);
    try {
      console.log('Making API call to verify gift card:', cardNumber);
      const response = await api.get(`/gift-cards/${cardNumber}/balance`);
      const { balance } = response.data;
      
      setGiftCardBalance(balance);
      setGiftCardVerified(true);
      setGiftCardAmount(Math.min(balance, calculateTotal())); // Auto-set to min of balance or total
      toast.success(`Gift card verified! Balance: ${Math.round(balance)} RWF`);
    } catch (error) {
      ...
      if (error.response?.status === 400) {
        const errorMsg = error.response.data?.error || 'Invalid gift card';
        if (errorMsg.includes('Invalid card number format')) {
          toast.error('Invalid gift card number format. Please check the number and try again.');
        } else if (errorMsg.includes('not active')) {
          toast.error('This gift card is not active or has expired.');
        } else {
          toast.error(errorMsg);
        }
      } else if (error.response?.status === 404) {
        toast.error('Gift card not found. Please check the number and try again.');
      } else {
        toast.error(error.response?.data?.error || 'Failed to verify gift card. Please try again.');
      }
    } finally {
      ...
      <html>
        ...
        <body>
          ...
          <div class="receipt-info">
            ...
            <div>Order Ref: ${currentOrderRef}</div>
            <div>Time: ${new Date().toLocaleTimeString('en-GB', { hour12: false })}</div>
            <div>Customer: ${selectedCustomer ? selectedCustomer.firstName + ' ' + selectedCustomer.lastName : 'Guest'}</div>
          ...
          <div class="footer">
            ...
            <div style="margin-top: 4px;">Generated: ${new Date().toLocaleString('en-GB')}</div>
          </div>
        </body>
      </html>
    `);
  ...
  const calculateRemainingAmount = () => {
    ...
    if (paymentMethod === 'gift_card' && giftCardVerified) {
  ...
  const validateOrder = () => {
    ...
    if (paymentMethod === 'gift_card') {
      if (!giftCardNumber || giftCardNumber.length !== 16) {
        newErrors.payment = 'Please enter a valid 16-digit gift card number';
      } else if (!giftCardVerified) {
        newErrors.payment = 'Please verify the gift card before proceeding';
      } else if (giftCardAmount <= 0) {
        newErrors.payment = 'Please enter an amount to use from the gift card';
      } else if (giftCardAmount > giftCardBalance) {
        newErrors.payment = 'Gift card amount cannot exceed available balance';
      } else if (giftCardAmount > calculateTotal()) {
        newErrors.payment = 'Gift card amount cannot exceed order total';
      }
    }
    ...
    if (isSplitPayment) {
      ...
      if (Number(totalSplit.toFixed(2)) !== Number(orderTotal.toFixed(2))) {
        if (totalSplit > orderTotal) {
          newErrors.splitPayment = `Total payment (${formatCurrency(totalSplit)}) cannot exceed order total (${formatCurrency(orderTotal)})`;
        } else {
          newErrors.splitPayment = `Total split payments (${formatCurrency(totalSplit)}) must match order total (${formatCurrency(orderTotal)})`;
        }
      }
    }
  ...
  const saveOrderToDatabase = async () => {
    ...
    try {
      ...
      const giftoPayment = !isSplitPayment && paymentMethod === 'gifto';
      ...
      // Handle gift card redemption if gift card payment is used
      if (paymentMethod === 'gift_card' && giftCardVerified && giftCardAmount > 0) {
        try {
          console.log('Redeeming gift card:', giftCardNumber, 'Amount:', giftCardAmount);
          console.log('Order response structure:', response.data);
          ...
          // For gift card redemption, we can pass null for shop_id and let the backend handle it
          // Or we can get the user's shop_id from the user context if available
          const redeemData = {
            amount: giftCardAmount,
            order_id: orderId
          };
          
          // Only include shop_id if we have a valid one
          // For now, omit shop_id to let backend handle it
          
          await api.post(`/gift-cards/${giftCardNumber}/redeem`, redeemData);
          toast.success(`Gift card redeemed: ${Math.round(giftCardAmount)} RWF`);
          
          // Refresh gift card balance after successful redemption
          try {
            const balanceResponse = await api.get(`/gift-cards/${giftCardNumber}/balance`);
            const newBalance = balanceResponse.data.balance;
            setGiftCardBalance(newBalance);
            console.log(`Gift card balance updated to: ${newBalance} RWF`);
            
            // Reset gift card amount for next use
            setGiftCardAmount(0);
          } catch (balanceError) {
            console.error('Failed to refresh gift card balance:', balanceError);
          }
        } catch (giftCardError) {
          console.error('Gift card redemption error:', giftCardError);
          console.error('Gift card error details:', giftCardError.response?.data);
          toast.error('Order saved but gift card redemption failed. Please contact support.');
        }
      }
      ...
      <>
        <div className="flex flex-col xl:flex-row h-full w-full bg-white shadow-sm border border-gray-200 overflow-hidden">
          ...
          <div className="w-full xl:w-2/3 p-2 sm:p-3 lg:p-4 flex flex-col gap-3 sm:gap-4 overflow-y-auto bg-gradient-to-br from-gray-50 to-white">
            {/* Customer Selection */}
            <div>
              {/* Sales Records Navigation */}
              <button
                onClick={onClose}
                className="w-auto mb-3 flex items-center justify-center gap-2 bg-primary-600 text-white border border-transparent hover:bg-primary-700 font-medium py-2 px-4 rounded-xl shadow-sm transition-all duration-200 group"
              >
            ...
            <div>
              <label className="block text-sm sm:text-base font-semibold text-gray-700 mb-2">
                <Package className="h-4 w-4 sm:h-5 sm:w-5 inline mr-2" />Add Products
              </label>
              {/* Barcode Scanner Section */}
              <div className="mb-3 sm:mb-4 p-3 sm:p-4 bg-primary-50 border border-primary-200 rounded-xl shadow-sm">
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 mb-2">
                  <div className="flex items-center space-x-2">
                    <Scan className="h-4 w-4 sm:h-5 sm:w-5 text-blue-600" />
                    <span className="text-sm sm:text-base font-semibold text-blue-800">Quick Add by Barcode</span>
                  </div>
                ...
                <div className="flex flex-col sm:flex-row gap-2">
                  ...
                  <div className="flex gap-2">
                    <button
                      ...
                      disabled={isScanning}
                      className="flex-1 sm:flex-none px-3 sm:px-4 py-2 sm:py-3 bg-primary-600 text-white rounded-lg hover:bg-primary-700 focus:ring-2 focus:ring-primary-500 disabled:opacity-50 transition-colors"
                ...
                <p className="text-xs text-blue-600 mt-1">
                  💡 Tip: Products are automatically added when a valid barcode is scanned or entered
                </p>
              </div>
                ...
                {(productSearch || barcodeSearch) && (
                  <div className="absolute top-full left-0 right-0 bg-white border border-gray-200 rounded-xl shadow-lg max-h-64 overflow-y-auto z-10">
                    ...
                    {sortedProducts.length === 0 && (
                      <p className="p-3 sm:p-4 text-gray-500 text-center text-sm">No products found</p>
                    )}
                  </div>
                )}
              </div>
              ...
              <div className="mb-4 hidden sm:block">
                <div className="flex items-center justify-between mb-3">
                  <h3 className="text-lg sm:text-xl font-semibold text-gray-900">All Available Products</h3>
                  <div className="flex items-center gap-2">
                    <label htmlFor="sortField" className="text-xs text-gray-500">Sort by</label>
                    <select
                      id="sortField"
                      value={sortField}
                          ...
                          {(() => {
                            ...
                            return availableStock > 0 && (
                              <button
                                ...
                                className="px-3 rounded-lg text-sm font-medium bg-primary-100 text-primary-700 hover:bg-primary-200"
                                title="Quick add 1"
          ...
          <div id="cart-container" className="w-full xl:w-1/3 p-2 sm:p-3 lg:p-4 flex flex-col gap-3 sm:gap-4 bg-gradient-to-br from-slate-50 to-gray-100 border-t xl:border-t-0 xl:border-l border-gray-200 h-full relative">
              ...
              <div className="border border-gray-200 rounded-xl max-h-48 sm:max-h-64 overflow-y-auto bg-white shadow-sm">
                {orderItems.length === 0 ? (
                  <div className="p-4 sm:p-6 text-center text-gray-500">
                    ...
                    <p className="text-xs sm:text-sm">Search for products to add them to the order</p>
                  </div>
                ) : (
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center space-x-2">
                            ...
                            {item.addedViaBarcode && (
                              <span className="inline-flex items-center px-1.5 sm:px-2 py-0.5 rounded text-xs font-semibold bg-primary-100 text-primary-800">
            ...
            {orderItems.length > 0 && (
              <div className="bg-white rounded-xl p-3 sm:p-4 shadow-sm border border-gray-200">
                <h3 className="font-semibold text-gray-900 mb-2 sm:mb-3 text-base sm:text-lg">Order Summary</h3>
            ...
            <div>
              <div className="flex items-center justify-between mb-2">
                <label className="block text-sm sm:text-base font-semibold text-gray-700">
                  <CreditCard className="h-4 w-4 sm:h-5 sm:w-5 inline mr-2" />Payment Method
                </label>
                <div className="flex items-center">
                  <input
                    type="checkbox"
                  ...
                  />
                  <label htmlFor="split-payment" className="ml-2 text-sm text-gray-700 cursor-pointer">
                    Split Payment
                  </label>
                </div>
              </div>
              ...
              {!isSplitPayment ? (
                ...
                >
                  ...
                  <option value="gifto">Gifto (Free)</option>
                </select>
              ) : (
                <div className="space-y-3 bg-gray-50 p-3 rounded-xl border border-gray-200">
                    ...
                    <React.Fragment key={index}>
                      <div className="flex items-center space-x-2">
                        ...
                        <input
                          ...
                          onChange={(e) => {
                            ...
                            const maxAllowed = Math.max(0, orderTotal - otherPayments);

                            const newPayments = [...splitPayments];
                            // Cap the value if it exceeds what's remaining
                            newPayments[index].amount = val > maxAllowed ? maxAllowed : val;
                            setSplitPayments(newPayments);
                          }}
                          className="w-1/3 px-2 py-2 border border-gray-300 rounded-lg text-sm"
                          placeholder="Amount"
                          max={Math.max(0, calculateTotal() - splitPayments.reduce((sum, p, i) => i === index ? sum : sum + (parseFloat(p.amount) || 0), 0))}
                        />
                        <button
                          type="button"
                          onClick={() => {
                      ...
                      {index === splitPayments.length - 1 && (
                        <div className="text-right text-xs text-gray-500 mt-1 mb-2">
                          Remaining: {formatCurrency(Math.max(0, calculateTotal() - splitPayments.reduce((sum, p) => sum + (parseFloat(p.amount) || 0), 0)))}
                        </div>
                      )}
                    </React.Fragment>
                  ))}
                  <div className="flex justify-between items-center pt-2">
                    ...
                    <div className="text-right text-sm">
                      ...
                      <span className={Math.abs(splitPayments.reduce((sum, p) => sum + (parseFloat(p.amount) || 0), 0) - calculateTotal()) < 1 ? "text-green-600 font-bold" : "text-red-600 font-bold"}>
                        {formatCurrency(splitPayments.reduce((sum, p) => sum + (parseFloat(p.amount) || 0), 0))}
                      </span>
                    </div>
                  </div>
                </div>
              ...
              {(errors.payment || errors.splitPayment) && (
                <p className="text-red-600 text-sm sm:text-base mt-1">{errors.payment || errors.splitPayment}</p>
              )}
            </div>

            {/* Gifto Payment Notice */}
            {paymentMethod === 'gifto' && !isSplitPayment && (
              <div className="bg-green-50 p-4 rounded-xl border border-green-200">
                <p className="text-sm font-semibold text-green-800">🎁 Gifto Payment — Customer pays RWF 0</p>
                <p className="text-xs text-green-600 mt-1">The order total will be recorded as 0 RWF.</p>
              </div>
            )}

            {/* Gift Card Details - Show only when gift card is selected */}
                ...
                <div className="space-y-3">
                  ...
                  <div>
                    ...
                    <div className="flex space-x-2">
                      ...
                      <button
                        type="button"
                        onClick={() => verifyGiftCard(giftCardNumber)}
                        disabled={checkingGiftCard || giftCardNumber.length !== 16}
                      ...
                      >
                        {checkingGiftCard ? 'Checking...' : 'Verify'}
                      </button>
                    </div>
                  </div>
                  ...
                  {giftCardVerified && (
                    <div className="bg-green-50 p-3 rounded-lg border border-green-200">
                      <div className="flex justify-between items-center mb-2">
                        <span className="text-sm font-medium text-green-800">Card Balance:</span>
                        <span className="text-sm font-bold text-green-900">{Math.round(giftCardBalance)} RWF</span>
                      </div>
                        ...
                        <input
                          ...
                          onChange={(e) => {
                            const value = Math.min(
                              ...
                              Math.min(giftCardBalance, calculateTotal())
                            );
                            setGiftCardAmount(value);
                          }}
                          min="0"
                          max={Math.min(giftCardBalance, calculateTotal())}
                          className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent text-sm"
                          placeholder="0"
                        />
                        <p className="text-xs text-gray-500 mt-1">
                          Max: {Math.round(Math.min(giftCardBalance, calculateTotal()))} RWF
                        </p>
                      </div>

                      {/* Remaining Amount */}
                      {giftCardAmount > 0 && calculateRemainingAmount() > 0 && (
                        <div className="mt-2 p-2 bg-yellow-50 border border-yellow-200 rounded">
                          <p className="text-sm text-yellow-800">
                            <strong>Remaining to pay:</strong> {Math.round(calculateRemainingAmount())} RWF
                          </p>
            ...
            <div>
              <label className="block text-sm sm:text-base font-semibold text-gray-700 mb-2">
                <AlertTriangle className="h-4 w-4 sm:h-5 sm:w-5 inline mr-2" />Payment Status
              </label>
              <div className="space-y-3">
                <label className="flex items-center space-x-3 cursor-pointer">
                  ...
                  />
                  <div>
                    <span className="text-sm sm:text-base font-medium text-gray-900">Complete Payment</span>
                    <p className="text-xs text-gray-500">Customer pays full amount, product removed from stock</p>
                  </div>
                </label>
                <label className="flex items-center space-x-3 cursor-pointer">
                  <input
                  ...
                  />
                  <div>
                    <span className="text-sm sm:text-base font-medium text-gray-900">Partial Payment (Reservation)</span>
                    <p className="text-xs text-gray-500">Customer pays 50%, product reserved until full payment</p>
                  </div>
                </label>
              </div>
            ...
            {/* Notes */}
            <div>
              <label className="block text-sm sm:text-base font-semibold text-gray-700 mb-2">Notes (Optional)</label>
              <textarea
                value={notes}
            ...
            <div className="bg-white rounded-xl max-w-2xl w-full max-h-[95vh] overflow-y-auto">
              ...
              <div className="bg-primary-600 border-b-2 border-primary-700 p-6">
                <div className="flex items-center justify-between">
                  <div className="flex-1">
                    <h1 className="text-3xl font-bold text-white">URUTIROSE STORE</h1>
                  ...
                  <div className="text-right border-l-2 border-primary-500 pl-6">
                    ...
                    <div className="mt-2 text-sm text-primary-100 space-y-1">
                      ...
                      <p><span className="font-medium">Order Ref:</span> {orderReference || generateOrderReference()}</p>
                      <p><span className="font-medium">Date:</span> {new Date().toLocaleDateString('en-GB')}</p>
                      <p><span className="font-medium">Time:</span> {new Date().toLocaleTimeString('en-GB', { hour12: false })}</p>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="p-6" id="invoice-content">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                    ...
                    <div className="space-y-2">
                      <div className="flex justify-between">
                        ...
                        <span className="font-medium">
                          {selectedCustomer ? `${selectedCustomer.first_name} ${selectedCustomer.last_name}` : 'Guest Customer'}
                        </span>
                      </div>
                      {selectedCustomer && (
                        <>
                      ...
                      <div className="flex justify-between">
                        ...
                        <span className={`font-medium capitalize ${paymentStatus === 'pending' ? 'text-orange-600' : 'text-green-600'
                          ...
                          {paymentStatus === 'pending' ? 'Partial Payment (50%)' : 'Complete Payment'}
                        </span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="bg-gray-50 rounded-lg p-4">
                    <h4 className="font-semibold text-gray-900 mb-3 text-lg">Order Information</h4>
                    <div className="space-y-2">
                      <div className="flex justify-between">
                ...
                <div className="bg-gray-50 rounded-lg p-6">
                  <h4 className="font-semibold text-gray-900 mb-4 text-lg">Financial Summary</h4>
      ...
      <div className={isFullPage
        ...
        <div className="p-3 sm:p-4 lg:p-6 grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6">
              ...
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center space-x-2">
                  ...
                  <span className="text-xs text-gray-500 bg-gray-100 px-2 py-0.5 rounded-full">
                    {customersLoading ? 'Loading...' : `${customers.length} customers`}
                  </span>
                </div>
                <button
              ...
              <div className="mb-3 p-3 bg-primary-50 border border-primary-200 rounded-lg">
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center space-x-2">
                    ...
                    <span className="text-sm font-medium text-blue-800">Quick Add by Barcode</span>
                  </div>
                  <div className="text-xs text-blue-600">
                    <kbd className="px-1 py-0.5 bg-white border border-blue-300 rounded text-xs">F2</kbd> Focus |
                    <kbd className="px-1 py-0.5 bg-white border border-blue-300 rounded text-xs ml-1">F3</kbd> Scan
                ...
                <div className="flex space-x-2">
                  ...
                  <button
                    onClick={simulateBarcodeScanner}
                    disabled={isScanning}
                    className="px-3 py-2 bg-primary-600 text-white rounded-md hover:bg-primary-700 focus:ring-2 focus:ring-primary-500 disabled:opacity-50 transition-colors"
                    title="Activate barcode scanner (F3)"
                  >
          ...
          <div className="space-y-6">
            ...
            {/* Gift Card Details - Compact Version */}
                  ...
                  <div>
                    ...
                    <div className="flex space-x-1">
                      <input
                        ...
                        placeholder="1234567890123456"
                        className="flex-1 px-2 py-1.5 border border-gray-300 rounded focus:ring-1 focus:ring-blue-500 focus:border-transparent font-mono text-xs"
                        maxLength={16}
                      />
                      <button
                        type="button"
                        onClick={() => verifyGiftCard(giftCardNumber)}
                        disabled={checkingGiftCard || giftCardNumber.length !== 16}
                        className="px-3 py-1.5 bg-blue-600 text-white rounded hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed text-xs font-medium"
                      >
                        {checkingGiftCard ? 'Checking...' : 'Verify'}
                      </button>
                    </div>
                  ...
                  {giftCardVerified && (
                    <div className="bg-green-50 p-2 rounded border border-green-200">
                      <div className="flex justify-between items-center mb-1">
                        <span className="text-xs font-medium text-green-800">Balance:</span>
                        <span className="text-xs font-bold text-green-900">{Math.round(giftCardBalance)} RWF</span>
                      </div>
                      ...
                      <div>
                        ...
                        <input
                          ...
                          min="0"
                          max={Math.min(giftCardBalance, calculateTotal())}
                          className="w-full px-2 py-1.5 border border-gray-300 rounded focus:ring-1 focus:ring-blue-500 focus:border-transparent text-xs"
                          placeholder="0"
                        />
                      ...
                      {giftCardAmount > 0 && calculateRemainingAmount() > 0 && (
                        <div className="mt-1 p-1.5 bg-yellow-50 border border-yellow-200 rounded">
                          <p className="text-xs text-yellow-800">
                            <strong>Remaining:</strong> {Math.round(calculateRemainingAmount())} RWF
                          </p>
                        </div>
                      )}
                    </div>
